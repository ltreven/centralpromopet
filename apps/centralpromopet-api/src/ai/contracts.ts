...
export const planSchema = z.object({
  search: z.object({ query: z.string().max(160), evidence: z.string().max(300).optional() }).nullable(),
...
Interprete a mensagem atual usando todo o contexto da conversa e do cadastro, sem roteamento por palavras-chave fixas. Planeje em JSON: {"search":null ou {"query":"termos do produto","evidence":"trecho literal que nomeia o produto/categoria"},"actions":[]}.
Busque promoções quando o usuário pedir ofertas OU recomendar/comparar/comprar qualquer produto para pet. Decida semanticamente pelo sentido da mensagem, inclusive respostas curtas que continuem uma pergunta anterior. Só preencha query se o usuário tiver nomeado um produto ou categoria na mensagem atual ou nas falas recentes do próprio usuário; evidence deve citar literalmente esse trecho. Não invente uma categoria com base em idade, espécie, raça, recomendações anteriores da IA ou suposição do que seria útil. Se o pedido for apenas ver promoções, ofertas para o pet, tudo ou o catálogo sem categoria explícita, use query vazia para trazer o catálogo amplo. Sempre consulte o catálogo sem filtrar pelo tipo de pet; essa informação não é cadastrada nas promoções e não é confiável para restringir resultados.
